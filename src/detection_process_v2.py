...
def get_out_detection(encoder_, in_detection,file_img):  # done----------------------------------------------------------------------------------------------------------
  ...
  for idxFrame in range(frame_indMin, frame_indMax + 1):
      ...
      features = encoder_(img_bgr, rows1[:, 2:6].copy())

      out_detections = out_detections+ [np.r_[(row, feature)] for row, feature in zip(rows1, features)]
      print(file_img[idxFrame], "Frame", idxFrame, "Done.")

  return out_detections


def produce_detection(encoder_, dir_mot, dir_out, dir_detection=None):  # done-------------------------------------------------------------------
    ...
    for sequence in os.listdir(dir_mot):
        ...
        out_detections=get_out_detection(encoder_, in_detection,file_img)
